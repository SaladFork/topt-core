...
export class FacilityAPI {
  ...
  public static getByID(facilityID: string): Promise<Facility | null> {
    if (FacilityAPI._pending.has(facilityID)) {
      log.trace(
        `${facilityID} already has a pending request, using that one instead`
      )
    ...
    const prom = new Promise<Facility | null>(async (resolve, reject) => {
      ...
      if (FacilityAPI._cache.has(facilityID)) {
        ...
        try {
          ...
          const request: ResponseContent<any> = await CensusAPI.get(
            url
          ).promise()
          ...
          if (request.code == 200) {
            ...
            const facility: Facility = FacilityAPI.parse(
              request.data.map_region_list[0]
            )
  ...
  public static getByIDs(IDs: string[]): Promise<Facility[]> {
    return new Promise<Facility[]>(async (resolve, reject) => {
      ...
      const url: string = `/map_region?facility_id=${requestIDs.join(
        ','
      )}&c:limit=100`
          ...
          if (request.code == 200) {
            if (request.data.returned == 0) {
              ...
              for (const facID of requestIDs) {
                ...
                if (elem == undefined) {
                  log.debug(
                    `Failed to find Facility ID ${facID}, settings cache to null`
                  )
